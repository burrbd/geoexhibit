...
class SteelThreadValidator:
    ...
    def validate_api_gateway_cors(self) -> bool:
        ...
        try:
            ...
            if cors_origin and ('GET' in str(cors_methods) or response.status_code == 200):
                print("✅ API Gateway with CORS: PASSED")
                print(f"   CORS Origin: {cors_origin}")
                return True
            else:
                print(f"❌ API Gateway CORS: Missing headers - Origin: {cors_origin}, Methods: {cors_methods}")
                return False
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'assets' in data and 'type' in data:
                    ...
                    print(f"   Demo STAC item accessible: {len(data.get('assets', {}))} assets")
                    return True
