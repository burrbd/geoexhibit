    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if all(field in data for field in required_fields):
                ...
                print(
                    f"   Tiles template: {data['tiles'][0] if data['tiles'] else 'None'}"
                )
                return True
...
def validate_tile_request(cloudfront_url: str, stac_item_url: str) -> bool:
    ...
    tile_url = (
        f"{cloudfront_url}/stac/tiles/8/128/128.png?url={encoded_url}&format=webp"
    )
...
def main():
    ...
    if len(sys.argv) != 2:
        ...
        print(
            "Example: python validate-infrastructure.py https://d1234567890.cloudfront.net"
        )
        sys.exit(1)
