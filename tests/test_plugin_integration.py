...
def test_multiple_analyzer_registration():
    ...
    class MultiTest1(Analyzer):
        ...
        def analyze(self, feature: Dict[str, Any], timespan: TimeSpan) -> AnalyzerOutput:
            return AnalyzerOutput(
                primary_cog_asset=AssetSpec(key="m1", href="/tmp/m1.tif", roles=["data"])
            )
    ...
    class MultiTest2(Analyzer):
        ...
        def analyze(self, feature: Dict[str, Any], timespan: TimeSpan) -> AnalyzerOutput:
            return AnalyzerOutput(
                primary_cog_asset=AssetSpec(key="m2", href="/tmp/m2.tif", roles=["data"])
            )
...
def test_config_json_example():
    ...
    config_json = {
        ...
        "map": {
            "pmtiles": {"minzoom": 5, "maxzoom": 14},
            "base_url": ""
        },
        "stac": {
            "use_extensions": ["proj", "raster"],
